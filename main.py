...
async def admin_panel(message: types.Message):
    logger.info(f"Админ-панель запрошена пользователем {message.from_user.id}")
...
async def admin_create(callback: types.CallbackQuery):
    logger.info(f"Админ {callback.from_user.id} создает заявку")
...
async def call_send_number(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"📞 Пользователь {user_id} нажал 'Сдать номер'")
        ...
        if user and user["current_number"]:
            ...
            if last_time:
                ...
                if elapsed < 600:
                    ...
                    logger.info(f"Пользователь {user_id} пытается сдать номер повторно, осталось {remaining} сек")
                    await bot.send_message(
                        user_id,
    ...
    logger.info(f"Состояние установлено: {current_state} для пользователя {user_id}")
...
async def timeout_number(user_id: int, state: FSMContext):
    logger.info(f"Запущен таймаут для пользователя {user_id}")
    await asyncio.sleep(60)
    current_state = await state.get_state()
    if current_state == Form.waiting_number.state:
        data = await state.get_data()
        if data.get("user_id") == user_id:
            logger.info(f"Таймаут сработал для пользователя {user_id}")
            await state.clear()
...
async def process_number(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Обработка номера: user_id={user_id}, number={number}, username={username}")
...
async def request_sms(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Админ запросил SMS для пользователя {user_id}")
...
async def timeout_sms(user_id: int, state: FSMContext):
    logger.info(f"Запущен таймаут SMS для пользователя {user_id}")
    await asyncio.sleep(60)
    current_state = await state.get_state()
    if current_state == Form.waiting_sms.state:
        data = await state.get_data()
        if data.get("user_id") == user_id:
            logger.info(f"Таймаут SMS сработал для пользователя {user_id}")
            await state.clear()
            await bot.send_message(user_id, "<b>⏰ Время вышло. Заявка отменена</b>", parse_mode="HTML")
...
async def reject_request(callback: types.CallbackQuery):
    ...
    logger.info(f"Админ отклонил заявку пользователя {user_id}")
...
async def process_sms(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Получен SMS код от пользователя {user_id}: {sms_code}")
    
    data = await state.get_data()
    if data.get("user_id") != user_id:
        logger.warning(f"Несоответствие user_id: ожидался {data.get('user_id')}, получен {user_id}")
        return
    ...
    logger.info(f"SMS код отправлен админу")
...
async def number_accepted(callback: types.CallbackQuery):
    ...
    logger.info(f"Админ принял номер пользователя {user_id} с кодом {sms_code}")
    ...
    await callback.message.delete_reply_markup()
    logger.info(f"Пользователю {user_id} начислено 4 USDT")

@dp.callback_query(F.data.startswith("registered_"))
async def number_registered(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[1])
    logger.info(f"Админ отметил номер пользователя {user_id} как зарегистрированный")
...
async def got_error(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[1])
    logger.info(f"Админ отметил ошибку для пользователя {user_id}")
    ...
    async def get_error_reason(message: types.Message):
        if message.from_user.id != ADMIN_ID:
            return
        reason = message.text
        logger.info(f"Админ отправил причину ошибки: {reason}")
...
async def cancel_sms(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    logger.info(f"Пользователь {user_id} отменил ввод SMS")
